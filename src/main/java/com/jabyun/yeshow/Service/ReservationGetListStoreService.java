...
@Component("ReservationGetListStoreService")
public class ReservationGetListStoreService extends YeshowService{
	...
	public ArrayList<Object> execute(JSONObject dataObject) throws SQLException, DuplicateException {
		...
		int month=Integer.parseInt((String)dataObject.get("month"));
		int day=Integer.parseInt((String)dataObject.get("day"));
		java.sql.Date dateToday = new java.sql.Date (Calender.getDate(year, month, day));
		System.out.println(dateToday.toString());
		List<Reservation> reservationList=reservationRepository.findByStoreToday(store,dateToday);
